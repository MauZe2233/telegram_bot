...
async def process_callback_button1(callback_query: types.CallbackQuery, bot : Bot, state:FSMContext):
    ...
    await bot.send_message(chat_id=u_id, text = "Введите адрес заявки", reply_markup=types.ReplyKeyboardRemove())
    await state.update_data(zone=callback_query.data)
    await state.set_state(Request.request_text)

@router.message(Request.request_text, F.text)
...
async def info(message: types.Message, state: FSMContext):
    ...
    await message.answer("Прикрепите фото проблемы\nМожете пропустить этот пункт", reply_markup=keyboard)
...
async def ans2(message: types.Message, state: FSMContext, bot: Bot):
    ...
    for el in my_reqs:
        ...
        try:
            ...
            await bot.send_photo(chat_id=u_id, photo='https://upload.wikimedia.org/wikipedia/commons/thumb/3/37/Flag_of_the_New_People_%28political_party%29.svg/2560px-Flag_of_the_New_People_%28political_party%29.svg.png')
...
@router.message(Request.request_photo, F.text.lower() == 'пропустить')
async def get_photo(message: types.Message, state: FSMContext):
    ...
    for el in list:
        cur.execute(
            "INSERT INTO requests (name, phone, tg, text, photo, status, zone) VALUES ('%s', '%s', '%s', '%s', '%s', 'Ожидает рассмотрения', '%s')" % (el[1], el[2], el[3], data["request"], None, location)
        )
...
async def get_photo(message: types.Message, state: FSMContext):
    ...
    for el in list:
        cur.execute(
            "INSERT INTO requests (name, phone, tg, text, photo, status, zone) VALUES ('%s', '%s' ,'%s', '%s', '%s', 'Ожидает рассмотрения', '%s')" % (el[1], el[2], el[3], data["request"], data["photo_file_id"], location)
        )
